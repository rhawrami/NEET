...
def get_ACS(five_year = True, years = 2023, vars = ['age'], filename = '', filepath = ''):
    """collects IPUMS CPS data and downloads extract
    ...
    if five_year:
        cleaned_samples_list = [f'us{yr}c' for yr in range(start, end + 1)]
    elif not five_year:
        cleaned_samples_list = [f'us{yr}a' for yr in range(start, end + 1)]

    extract = MicrodataExtract(
        collection='usa',
        samples=cleaned_samples_list,
        description='ACS Extract for NEETs',
        variables=vars
    )
    ...
    # rename files from cps_0000[X].[dat.gz/xml] to our filename parameter
    default_data_files = Path(glob.glob(filepath + '/' 'usa_0*.dat.gz')[0])
    default_ddi_files = Path(glob.glob(filepath + '/' 'usa_0*.xml')[0])
...
if __name__ == '__main__':
    ...
    get_ACS(five_year=True,years=(2022,2023), vars=['AGE', 'SEX'], filename='test_acs', filepath='datasets')